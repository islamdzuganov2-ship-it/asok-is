...
/**
 * Пара «экономия / издержки» — тип денежного потока, отдельная ось от RAG (severity).
 * Кредит (то, что прибавляет к цифре — эффект, доход, экономия) красится зелёным; дебет
 * (то, что её забирает — вложения, потери, издержки) — розовым. Не путать с RAG.good/bad:
 * высокий ALE — это «плохо» по severity, но сама цифра ALE как денежный поток — «издержки»,
 * не «низкий уровень».
 *
 * Разведены на light/dark явно (не через var()), как акценты в TileCard: единый статичный
 * розовый не проходит AA сразу в обеих темах — насыщенный текстовый тон, читаемый на белом
 * (light/classic), даёт ~2.7:1 на графитовой карточке, а светлый тон для графита проваливает
 * контраст на белом. Числа проверяются `npm run check:contrast`.
 */
...
/**
 * Служебные оттенки вне RAG-шкалы: они не про «хорошо/плохо», а про принадлежность
 * (тип задачи, категория сбоя). Раньше были literal-ами в 30+ местах — из-за этого один
 * и тот же смысл красился по-разному от экрана к экрану (UI-03).
 *
 * Пара `color`/`strong` — тот же инвариант, что у RAG: графика ≥3:1, текст и плашки ≥4.5:1.
 */
export const ACCENT = {
  /** Сине-стальной: «в работе», инфраструктура, нейтральный акцент. */
  slate:  { color: '#6E89A6', strong: '#56799F' },
  /** Фиолетовый: эскалация, «привнесено релизом». */
  violet: { color: '#7E57C2', strong: '#6A45AB' },
};
