...
export interface Proposal {
  ...
  // ТЗ v17 (req 6, роль «Исполнитель»): уточнения исполнителя по метрике/поручению и запрос на
  // перенос срока с обоснованием. Всё это «падает» менеджеру по качеству (он видит и решает).
...
// ─────────── Роль «Исполнитель» (req 6): уточнения + запрос переноса срока ───────────
...
/**
 * Поручения, назначенные на исполнителя (req 6): меры видимого набора, где owner совпадает с ФИО.
