...
describe('кокпит: текст без инженерного жаргона', () => {
  ...
  it('ceoFallback: блокирующие несоответствия', () => {
    const b: CockpitBundle = {
      ...emptyBundle,
      costDashboard: {
        portfolioAle: 1_000_000, risksCount: 5, degradationTotal: 0, nonconformitiesTotal: 10, verified: 8,
        closureRate: 80, blockingCount: 2,
        verdict: { eliminate: 1, compensate: 1, accept: 0 }, topRisks: [], bySystem: [],
      },
    };
  ...
  it('ceoFallback: просроченные меры', () => {
    const b: CockpitBundle = {
      ...emptyBundle,
      costDashboard: {
        portfolioAle: 1_000_000, risksCount: 5, degradationTotal: 0, nonconformitiesTotal: 10, verified: 10,
        closureRate: 100, blockingCount: 0,
        verdict: { eliminate: 0, compensate: 0, accept: 0 }, topRisks: [], bySystem: [],
      },
      overdueSummary: {
  ...
  it('ceoFallback: устойчивый портфель (ветка по умолчанию)', () => {
    const b: CockpitBundle = {
      ...emptyBundle,
      costDashboard: {
        portfolioAle: 200_000, risksCount: 3, degradationTotal: 0, nonconformitiesTotal: 4, verified: 4,
        closureRate: 100, blockingCount: 0,
        verdict: { eliminate: 0, compensate: 0, accept: 4 }, topRisks: [], bySystem: [],
      },
    };
