...
> **Деловой язык вывода для ЛПР (2026-08-16).** Реальный вывод конвейера показывал CIO/CTO/CEO
> формулировки внутренней инженерной кухни: «Grounding-контроль пройден», «Этапы на
> детерминированном fallback: E1, E2, E2Q, E3», «LLM-вывод недоступен/отбракован»,
> «[Severity>=High]» вместо названия правила. Читателю, не занимающемуся эксплуатацией системы,
> это неотличимо от отказа системы. Исправлено на уровне A (промпт), без дообучения весов:
> `backend/app/modules/llm/style_guide.py` — глоссарий запрещённых терминов и их деловых
> замен, подмешивается в `BASE_HONESTY` (personas.py) и тем самым автоматически идёт в
> системный промпт КАЖДОЙ персоны для КАЖДОЙ модели. Детерминированный скелет (E4/E6/E7 в
> reasoning.py, ярлыки правил в gate.py, формулировки в decisions.py) переписан вручную тем же
> словарём — это не текст модели, глоссарий его не перепишет сам. Регресс стережёт
> `test_no_engineering_jargon_when_pipeline_is_fully_deterministic` и
> `test_no_engineering_jargon_across_all_personas` в `tests/test_reasoning.py` (тот же приём,
> что и `_FORBIDDEN_TERMS`/тест на терминологию Toyota в `principles.py`). Механика (движок
> правил → LLM → grounding → fallback) при этом никуда не делась — она осталась в коде и в
> «Ходе рассуждения» (аудируемая трасса, для технических персон), просто не протекает в текст,
> обращённый к руководителю.
...
## 9. Многоаспектное аналитическое рассуждение — обновление знаний обучения (BL‑005)

Чтобы LLM мыслила **не однобоко** и выносила заключение руководителю только после разбора,
рассуждение построено как многоаспектный аналитический конвейер (опора на ISO 25010 / 38500).
Методология этапов — внутренний «скелет» логики; её терминология в user-facing вывод НЕ попадает
(только нейтральная аналитическая лексика — «рефакторинг v1», 2026‑07‑06).

Как это меняет **обучение** (в дополнение к уровням A/B/C выше):
- **A (промпт):** системный промпт задаёт конвейер факты входа → первопричина → ролевые точки
  зрения → контроль достоверности (grounding) → саморефлексия → заключение ЛПР; few-shot содержит
  пример с трассой. Промпты обогащаются доменным глоссарием ISO 25010 (`modules/llm/knowledge.py`).
...
5. **Драйверы: характеристики → подхарактеристики → метрики.** Для проблемных ИС спуститься
   ...
   Заключение без числового основания не выдаётся (grounding-контроль).
...
9. **Саморефлексия.** Перед выдачей проверить: каждое число взято из данных; охват и
...
- `backend/app/modules/llm/reasoning.py` — ✅ конвейер многоаспектного аналитического рассуждения
  (BL‑005): Э0–Э7, 4 линзы, grounding-фильтры (числа/плейсхолдеры/вырождение/якорение), доменный
  глоссарий (knowledge.py), fallback без модели; тесты `tests/test_reasoning.py`.
- `backend/app/modules/llm/style_guide.py` — глоссарий делового языка для вывода ЛПР (запрет
  инженерного жаргона: grounding/fallback/движок/трасса/коды этапов), подмешивается в
  `personas.BASE_HONESTY`.
