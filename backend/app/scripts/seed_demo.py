...
async def seed_data() -> None:
    async with AsyncSessionLocal() as db:
        ...
        # ДЕФ-38. Пароли демо-учёток лежат здесь ОТКРЫТЫМ ТЕКСТОМ — осознанно: это сид
        # демонстрационного стенда, логины публикуются в документации и нужны для показа
        # ролевой модели без администрирования. Риск закрыт с двух сторон:
        #   · seed_demo запускается вручную и только на демо-контуре;
        #   · settings.security_issues() не даёт стартовать в проде (DEMO_MODE=false) с
        #     дефолтными секретами, а DEMO_AUTH_BYPASS выключен по умолчанию (ДЕФ-02).
        # В продуктивной установке учётные записи заводит суперадминистратор через
        # «Администрирование → Пользователи»; этот скрипт там не выполняется.
        users_data = [
