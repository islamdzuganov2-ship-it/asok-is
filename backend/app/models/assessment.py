...
# Примечание: Убедитесь, что модель User импортирована где-то в проекте (например, в main.py),
# иначе ForeignKey("users.id") может вызвать ошибку при создании таблиц.
...
class AssessmentPeriod(Base, TimestampMixin):
    ...
    period = Column(String(20), nullable=False)  # Формат: Q1-2026
    status = Column(String(20), default="DRAFT")

    __table_args__ = (
        UniqueConstraint('system_id', 'period', name='uq_system_period'),
    )
...
class AssessmentValue(Base, TimestampMixin):
    ...
    # Атрибут expert_judgments будет добавлен через backref в ExpertJudgmentHistory
...
class ExpertJudgmentHistory(Base, TimestampMixin):
    ...
    # Внимание: Убедитесь, что таблица users существует до запуска миграций
