...
class EnterpriseProfile(Base, TimestampMixin):
    ...
    size_class: критерий по умолчанию — 209-ФЗ (микро/малое/среднее/крупное). Для банковского
    клиента 209-ФЗ структурно неприменим (ст. 4 ч. 3 закона прямо исключает кредитные организации
    из МСП) — вместо него используется место в рэнкинге банков РФ по чистым активам: топ-3→LARGE,
    топ 4-10→MEDIUM, ниже топ-15→SMALL. Решено 16.08.2026, см. §0 добавление к Р-7 в том же файле
    ТЗ. Места 11-15 рэнкинга правилом не покрыты — оставлены на ручное суждение аналитика.
    """
    __tablename__ = "enterprise_profile"

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    # Критерий по умолчанию — 209-ФЗ; для банков — рэнкинг по активам (В-28, см. docstring класса).
...
class MarketBenchmark(Base, TimestampMixin):
    ...
    со средним по рынку. В-30а закрыт 16.08.2026 (см. docs/ТЗ_19_Управленческий_Контур_и_Веса.md
    §0, добавление к Р-7) — источники согласованы, таблица заполнена `seed_market_benchmarks()`
    (service.py, идемпотентно, вызывается из main.py рядом с seed_econ_defaults под DEMO_MODE).
    ...
    Сеяные строки BENCHMARK_SUPPORT_RATE оставляют company_size_class пустым осознанно — рынок
    труда специалистов не сегментирован по размеру банка-работодателя ни в одном найденном
    источнике (грейд/регион — да, размер клиента — нет); `compare_support_rate()` откатывается на
    такую запись, когда под конкретный `EnterpriseProfile.size_class` данных нет, и помечает это
    в ответе, а не показывает молча цифру как размерно-специфичную.
    """
