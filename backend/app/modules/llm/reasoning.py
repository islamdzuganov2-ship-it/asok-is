...
def _facts_text(inp: ReasoningInput) -> str:
    ...
    Сработавшие правила контроля качества (rules_block) идут ПЕРВЫМИ: именно они — повод для
    объяснения, и LLM обязана объяснить их причины/риски/рекомендации (модель не решает, а
    объясняет вердикт).
    """
    sections = [
        ("Сработавшие правила контроля качества (повод для разбора)", _cap_block(inp.rules_block, 6)),
