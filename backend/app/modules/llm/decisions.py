...
TRUST_MATRIX: dict[tuple[str, str], TrustDecision] = {
    ("высокая", "низкая"): TrustDecision(
        "высокий", True,
        "вывод пригоден как основание решения; основания подробно расписаны в разборе по каждому пункту"),
    ("высокая", "средняя"): TrustDecision(
        "ограниченный", True,
        "вывод пригоден, но часть разделов заполнена по формальным правилам без содержательного "
        "анализа — проверить подробный разбор перед решением"),
    ("высокая", "высокая"): TrustDecision(
        "ограниченный", False,
        "большая часть разбора получена по формальным правилам, а не содержательным анализом: "
        "использовать как сводку фактов, а не как самостоятельный аналитический вывод"),
    ...
    ("средняя", "средняя"): TrustDecision(
        "ограниченный", False,
        "неполные данные и часть разбора без содержательного анализа: дособрать источники перед "
        "вынесением на правление"),
