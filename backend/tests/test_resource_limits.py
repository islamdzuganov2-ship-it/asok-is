...
def _find_compose() -> Path:
    ...
    Тесты гоняются и с хоста (корень репозитория на два уровня выше), и внутри контейнера,
    где смонтирован только ./backend → /app. Во втором случае манифест подкладывается
    отдельным read-only томом в /deploy/docker-compose.yml (см. docker-compose.yml), иначе
    страж молча пропускался бы именно там, где его обычно запускают.
    """
    env = os.getenv("ASOK_COMPOSE_PATH")
    if env:
        return Path(env)
    mounted = Path("/deploy/docker-compose.yml")
    if mounted.is_file():
        return mounted
    ...
    for parent in here.parents:
        ...
        if candidate.is_file():
            return candidate
    return here.parents[2] / "docker-compose.yml"
...
def test_res08_base_manifest_has_no_reload(compose: dict) -> None:
    """RES-08: в базовом манифесте нет --reload.

    Reloader держит второй процесс и перезапускает воркер на любое изменение файла —
    вместе с ним заново грузятся ~7 ГБ весов. Замер на стенде: первый запрос
    управленческого дашборда после рестарта не отвечал 120 с (фактически ~10 минут),
    повторный — 0.45 с. Для разработки флаг включается в docker-compose.override.yml.
    """
    command = compose["services"]["backend"]["command"]
    command = " ".join(command) if isinstance(command, list) else str(command)
    assert "--reload" not in command, (
        "RES-08: --reload в базовом манифесте. Каждый рестарт воркера заново грузит "
        "веса модели; это и выглядело как «утечка памяти»."
    )
